...
export default function SettingsPage() {
  const { user } = useAuth();
  const { clients, activeClient, refreshClients, loading: clientsLoading } = useClient();
  ...
  const loadAccount = async () => {
    ...
    try {
      const accountList = await refreshClients();
      const [subscriptionRes, walletRes] = tenantId
        ? await Promise.all([
            api.get(`/subscriptions/tenant/${tenantId}`).catch(() => ({ data: null })),
            api.get(`/wallet/${tenantId}`).catch(() => ({ data: null })),
          ])
        : [{ data: null }, { data: null }];
      const accountsForUsage = tenantId
        ? (accountList || []).filter((account) => idOf(account.tenantId) === tenantId)
        : activeClient
          ? [activeClient]
          : (accountList || []);
      const usageResults = await Promise.all(
        accountsForUsage.map(async (account) => {
      ...
      setAccountCount(accountsForUsage.length);
  ...
  useEffect(() => { loadAccount(); }, [tenantId]);
  ...
  return (
    <AppShell allowedRoles={['admin', 'master']}>
      <PageHeader
        title="Settings"
        subtitle={activeClient ? `Manage ${activeClient.name} plan, wallet, WhatsApp numbers, and login security.` : 'Select a WhatsApp account to review client plan and wallet details.'}
        action={
      ...
      {loadingAccount && !subscription ? (
        ...
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard label="WhatsApp numbers" value={accountCount} icon={MessageCircle} color="#25D366" sub={`Limit ${fmtLimit(limitValue(plan, 'whatsappNumbers'))}`} />
          ...
          <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
            <Card className="p-5">
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{plan?.description || (tenantId ? 'Assign a plan to this client from the control panel.' : 'This WhatsApp account is not attached to a client tenant.')}</p>
                </div>
                {subscriptionStatus && (
            ...
            <Card className="p-5">
              ...
              <div className="space-y-3">
                <UsageRow label="WhatsApp numbers" used={accountCount} limit={limitValue(plan, 'whatsappNumbers')} />
          ...
          <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
            <Card className="p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <Link href="/master/connect-whatsapp">
                  <Button size="sm"><MessageCircle size={14} /> Add number</Button>
                </Link>
              </div>
              {!activeClient ? (
                <div className="rounded-lg border border-dashed border-border p-5 text-sm text-muted-foreground">
                  Select a WhatsApp account from the sidebar.
                </div>
              ) : !clients.length ? (
                <div className="rounded-lg border border-dashed border-border p-5 text-sm text-muted-foreground">
            ...
            <Card className="p-5">
              ...
              <Link href="/master/wallet" className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline">
